  ...
  if (!info) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <Link href="/gujarat" className="mt-4 inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
          <ArrowLeft className="size-4" /> Back to Gujarat
        </Link>
      </div>
    );
  }

  const related = getRelatedCities("gujarat", city);
  const breadcrumb = generateBreadcrumbSchema([
    { name: "Home", url: siteMetadata.siteUrl },
    { name: "Gujarat", url: `${siteMetadata.siteUrl}/gujarat` },
    { name: info.city, url: `${siteMetadata.siteUrl}/gujarat/${city}` },
  ]);
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
        <Link href="/gujarat" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors mb-8">
          <ArrowLeft className="size-4" /> Gujarat
        </Link>

        <div className="mb-10">
          <div className="flex items-center gap-2 mb-2">
            <MapPin className="size-4 text-muted-foreground" />
            <span className="text-sm text-muted-foreground">Gujarat</span>
          </div>
        ...
        {related.length > 0 && (
          <div className="border-t pt-8">
            <h2 className="text-base font-semibold mb-4">More cities in Gujarat</h2>
            <div className="grid gap-3 sm:grid-cols-2">
              {related.map((c) => (
                <Link key={c.city} href={`/gujarat/${slugify(c.city)}`} className="group border rounded-lg p-4 hover:border-foreground/30 transition-all">
