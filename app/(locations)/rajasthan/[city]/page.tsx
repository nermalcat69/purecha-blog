...
export async function generateStaticParams() {
  ...
  const cities = getStateCities("rajasthan") || {};
...
export async function generateMetadata({ params }: Props) {
  ...
  const info = getCityInfoFromSlugs("rajasthan", city);
  if (!info) return {};
  return generatePageMetadata({
    title: `Chai Culture in ${info.city}, Rajasthan | PurechaChai`,
    description: `Discover ${info.city}'s chai culture — ${info.teaSpecialty}. ${info.teaCulture.slice(0, 120)}...`,
    canonical: `/rajasthan/${city}`,
    keywords: [`${info.city} chai`, `chai in ${info.city}`, "rajasthan chai"],
  });
}
  ...
  if (!info) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <Link href="/rajasthan" className="mt-4 inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
          <ArrowLeft className="size-4" /> Back to Rajasthan
        </Link>
      </div>
    );
  }

  const related = getRelatedCities("rajasthan", city);
  const breadcrumb = generateBreadcrumbSchema([
    { name: "Home", url: siteMetadata.siteUrl },
    { name: "Rajasthan", url: `${siteMetadata.siteUrl}/rajasthan` },
    { name: info.city, url: `${siteMetadata.siteUrl}/rajasthan/${city}` },
  ]);
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
        <Link href="/rajasthan" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors mb-8">
          <ArrowLeft className="size-4" /> Rajasthan
        </Link>

        <div className="mb-10">
          <div className="flex items-center gap-2 mb-2">
            <MapPin className="size-4 text-muted-foreground" />
            <span className="text-sm text-muted-foreground">Rajasthan</span>
          </div>
        ...
        {related.length > 0 && (
          <div className="border-t pt-8">
            <h2 className="text-base font-semibold mb-4">More cities in Rajasthan</h2>
            <div className="grid gap-3 sm:grid-cols-2">
              {related.map((c) => (
                <Link key={c.city} href={`/rajasthan/${slugify(c.city)}`} className="group border rounded-lg p-4 hover:border-foreground/30 transition-all">
