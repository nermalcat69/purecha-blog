...
export async function generateMetadata({ params }: Props) {
  ...
  const info = getCityInfoFromSlugs("maharashtra", city);
  if (!info) return {};
  return generatePageMetadata({
    title: `Chai Culture in ${info.city}, Maharashtra | PurechaChai`,
    description: `Discover ${info.city}'s chai culture — ${info.teaSpecialty}. ${info.teaCulture.slice(0, 120)}...`,
    canonical: `/maharashtra/${city}`,
    keywords: [`${info.city} chai`, `chai in ${info.city}`, `${info.city} tea`, "maharashtra chai"],
  });
}
  ...
  if (!info) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <Link href="/maharashtra" className="mt-4 inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
          <ArrowLeft className="size-4" /> Back to Maharashtra
        </Link>
      </div>
    );
  }

  const related = getRelatedCities("maharashtra", city);
  const breadcrumb = generateBreadcrumbSchema([
    { name: "Home", url: siteMetadata.siteUrl },
    { name: "Maharashtra", url: `${siteMetadata.siteUrl}/maharashtra` },
    { name: info.city, url: `${siteMetadata.siteUrl}/maharashtra/${city}` },
  ]);
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
        <Link href="/maharashtra" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors mb-8">
          <ArrowLeft className="size-4" /> Maharashtra
        </Link>
        ...
        <div className="mb-10">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-muted-foreground">Maharashtra</span>
          </div>
        ...
        {related.length > 0 && (
          <div className="border-t pt-8">
            <h2 className="text-base font-semibold mb-4">More cities in Maharashtra</h2>
            <div className="grid gap-3 sm:grid-cols-2">
              {related.map((c) => (
                <Link
                  key={c.city}
                  href={`/maharashtra/${slugify(c.city)}`}
