  ...
  if (!info) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <Link href="/uttar-pradesh" className="mt-4 inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
          <ArrowLeft className="size-4" /> Back to Uttar Pradesh
        </Link>
      </div>
    );
  }

  const related = getRelatedCities("uttar-pradesh", city);
  const breadcrumb = generateBreadcrumbSchema([
    { name: "Home", url: siteMetadata.siteUrl },
    { name: "Uttar Pradesh", url: `${siteMetadata.siteUrl}/uttar-pradesh` },
    { name: info.city, url: `${siteMetadata.siteUrl}/uttar-pradesh/${city}` },
  ]);
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
        <Link href="/uttar-pradesh" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors mb-8">
          <ArrowLeft className="size-4" /> Uttar Pradesh
        </Link>

        <div className="mb-10">
          <div className="flex items-center gap-2 mb-2">
            <MapPin className="size-4 text-muted-foreground" />
            <span className="text-sm text-muted-foreground">Uttar Pradesh</span>
          </div>
        ...
        {related.length > 0 && (
          <div className="border-t pt-8">
            <h2 className="text-base font-semibold mb-4">More cities in Uttar Pradesh</h2>
            <div className="grid gap-3 sm:grid-cols-2">
              {related.map((c) => (
                <Link key={c.city} href={`/uttar-pradesh/${slugify(c.city)}`} className="group border rounded-lg p-4 hover:border-foreground/30 transition-all">
